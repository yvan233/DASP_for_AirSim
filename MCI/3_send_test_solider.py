...
solider_init_db = "solider_init.db"
solider_init_ip = "127.0.0.1"
solider_init_port = 10003
solider_record_db = "solider_record.db"

# 二、士兵通讯与控制
#-----------------------------------------------------------------------------------0
# 1. 与初始化接口通讯
# 注意：端口是10003
data_json = {"data":{"host":solider_init_ip,"port":solider_init_port,"db":solider_init_db,"db_table":task_name}} 
...
# 2. 与士兵数据存储接口通讯
data_json = {"data":{"db":solider_record_db,"record_all":1}} 
head = {"Content-Type": "application/json; charset=UTF-8", 'Connection': 'close'}
