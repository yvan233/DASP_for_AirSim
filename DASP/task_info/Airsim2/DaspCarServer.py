...
class DaspCarServer():
    def __init__(self, host = "localhost", port = 5000, UE_ip = "", vehicle_name = "", remote_ip = ""):
        ...
        self.clients= AirSimCarAgent(ip = UE_ip)
        self.threads = {}
        # self.threads["state"] = Thread(target=self.clients.updateState, args = (), daemon=True)
        # self.threads["video"] = Thread(target=self.clients.updateVideo, args = (), daemon=True)
        # self.threads["imu"] = Thread(target=self.clients.updateImu, args = (), daemon=True)
        # self.threads["lidar"] = Thread(target=self.clients.updateLidar, args = (), daemon=True)
