...
class AirSimCarAgent():
    ...
    def get_imu(self):
        # 获取车辆IMU数据
        imu_data = self.car.getImuData()
        imu_msg = {
            "timestamp":imu_data.time_stamp,
            "acc":self.airsim2minirobo(imu_data.linear_acceleration),
            "gyro":self.airsim2minirobo(imu_data.angular_velocity),
            "attitude": [180/math.pi*e for e in airsim.to_eularian_angles(imu_data.orientation)]
        }
        return imu_msg

    def airsim2minirobo(self, Vector3r):
        z = Vector3r.z_val
        y = Vector3r.y_val
        x = Vector3r.x_val
        y = -y
    ...
    def startUpload(self, parameter):
        ...
        if type == "video":
            ...
            thread = Thread(target=self.updateLidar, args = (), daemon=True)
            thread.start()
        elif type == "imu":
            self.upload_imu_flag = True
            self.imu_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.imu_socket.connect((remote_ip, port))
            thread = Thread(target=self.updateImu, args = (), daemon=True)
            thread.start()
        else:
            pass
    ...
    def updateLidar(self):
        ...
        while True:
            ...
            if lidar_data.size:
                thread = Thread(target=self.sendallAddHead, args=(self.lidar_socket, lidar_data.tobytes(), time_stamp,))
                thread.daemon = True
                thread.start()
            # if self.record_flag:
    ...
    def updateImu(self):
        ...
        while True:
            ...
            time.sleep(0.0001)
            thread = Thread(target=self.sendallAddHead, args=(self.imu_socket, json.dumps(imu_data).encode(), imu_data["timestamp"],))
            thread.daemon = True
            thread.start()
